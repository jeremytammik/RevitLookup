...
internal struct Win32Error
{
    ...
    public static readonly Win32Error ERROR_INVALID_WINDOW_HANDLE = new(1400);

    /// <summary>This operation returned because the timeout period expired.</summary>
    public static readonly Win32Error ERROR_TIMEOUT = new(1460);
    ...
    public static explicit operator HRESULT(Win32Error error)
    {
        ...
        if (error._value <= 0) return new HRESULT((uint) error._value);
    ...
    public override string ToString()
    {
        ...
        foreach (var publicStaticField in typeof(HRESULT).GetFields(BindingFlags.Static | BindingFlags.Public))
            if (publicStaticField.FieldType == typeof(HRESULT))
            {
                ...
                if (hr == this) return publicStaticField.Name;
            }

        // Try Win32 error codes also
        if (Facility == Facility.Win32)
            foreach (var publicStaticField in typeof(Win32Error).GetFields(BindingFlags.Static |
                                                                           BindingFlags.Public))
                if (publicStaticField.FieldType == typeof(Win32Error))
                {
                    var error = (Win32Error) publicStaticField.GetValue(null);
                    if ((HRESULT) error == this) return "HRESULT_FROM_WIN32(" + publicStaticField.Name + ")";
                }
