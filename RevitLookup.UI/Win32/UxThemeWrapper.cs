...
internal static class UxThemeWrapper
{
    ...
    {
        var themeState = _themeState; // capture latest state
        var needName = !themeChanged;
        ...
        while (needUpdate)
        {
            ...
            if (themeChanged)
            {
                ...
                var isActive = !SystemParameters.HighContrast && SafeNativeMethods.IsUxThemeActive();
            ...
            if (needUpdate)
            {
                ...
                var currentState = Interlocked.CompareExchange(ref _themeState, newState, themeState);
    ...
    {
        var themeNameSB = new StringBuilder(Win32Constant.MAX_PATH);
        var themeColorSB = new StringBuilder(Win32Constant.MAX_PATH);
                ...
                null, 0) == 0)
        {
            ...
            if (string.Compare(themeName, "aero", StringComparison.OrdinalIgnoreCase) == 0 && Utilities.IsOSWindows8OrNewer) themeName = "Aero2";
    ...
    private static bool IsActiveCompatWrapper => IsAppSupported ? _themeState.IsActive : _isActive;
    ...
    private static string ThemeNameCompatWrapper
    {
        get
        {
            if (IsAppSupported)
            {
                var themeState = EnsureThemeState(false);
